...
| 2026-03-31 | Sift killed after 3 iterations. Double constraints (shape+color) = cognitive overload; players collapse to one dimension. Hidden locks promising but only work if visible puzzle is completable. | Sift v1→v3: CI stuck at 0.2→0.6→0.6, puzzle never solved in 25+ moves |
| 2026-03-31 | Sort mercy-killed after v1. Bounded-reversal sorting has one dramatic collapse then tedious marching endgame (Breaks=1). 1D sequence puzzles lack spatial richness for daily depth. Strategic = intuitive outcome. | Sort v1: CI=0.2, par unreachable, strategic=intuitive (both 6 moves) |
| 2026-03-31 | Fold auto-killed. Irreversible damage (frozen stacks) is structurally incompatible with CI>0 — optimal path always avoids permanent penalties. For CI, "bad" states must be RECOVERABLE. | Fold v1: CI=0, Drama=0.0, greedy near-optimal |
| 2026-03-31 | Prune auto-killed. Subtraction is commutative — removal order among siblings irrelevant on trees. For tree-elimination depth, operations must be NON-COMMUTATIVE (division, modular arithmetic, value-dependent ops). | Prune v1: Skill-Depth=0%, CI=0, random=expert |
| 2026-03-31 | Ferry auto-killed. Edge swaps on constrained graphs are locally evaluable — sum-of-shortest-paths heuristic is near-perfect. For CI>0 in token permutation, operations must affect MORE than the two swapped elements (e.g., cyclic rotation of 3+ elements as in Loop CI=1.8). Pairwise swaps lack collateral damage. | Ferry v1: skillDepth=4.4%, CI=0, greedy=optimal on 4/5 puzzles |
| 2026-03-31 | Etch auto-killed. Irreversible contiguous path carving with exact targets is structurally monotonic — constraint-excess decreases with each correct removal. Difficulty is PATH PLANNING (computational), not STRATEGIC DECISION-MAKING (insightful). For daily puzzles, difficulty must come from strategy, not from search. | Etch v1: CI=0, drama=0.40, infoGain=1.14 |
| 2026-04-01 | Knot KEPT after 3 iterations. Loop drawing + hidden directional constraints = progressive revelation defeating A10. KEY: constraints must be LOAD-BEARING (force deviations from obvious topology), not decorative. v2 failed because compact loop passively satisfied generous constraints. v3 tightened constraint generation (cells at turns, not straight-throughs) and constraints became the actual puzzle. | Knot v1→v3: CI 1.8→1.0, skillDepth=100%, entropy=18.8 |
| 2026-04-01 | Chain auto-killed: DE=0.60. Sequential consumption with strict color-matching + rotation side effects creates A8 (forced path). State complexity != decision complexity. Rich state transformations are worthless if only ~1 cell matches target at each step. | Chain v1: DE=0.60, forced path despite complex rotation mechanics |
| 2026-04-01 | Sieve auto-killed: CI=0. Pure information-gathering games are structurally incompatible with CI>0 — every observation monotonically reduces uncertainty. No probe can ever be "counterintuitive" because no probe makes the state worse. For CI, actions must have COSTS (not just information value). | Sieve v1: CI=0, IGR=1.00, greedy sieving = random sieving |
